      ...
      .then(apiResponse => {
        return Promise.all(
          apiResponse.map(async pet => {
            ...
            const picUrl =
              pet.picKey && (await Storage.get(key, { level: "private" }));
    ...
    navigationOptions: props => {
      return {
        ...
        headerLeft: (
          <SideMenuIcon
            onPress={() =>
              props.screenProps.rootNavigator.navigate("DrawerOpen")
            }
          />
        )
      };
...
export default props => {
  ...
  return (
    <HomeNav screenProps={{ rootNavigator, ...screenProps, ...otherProps }} />
  );
};
